...
from lib.dataset_v4 import BalancedVideoDataset
...
TRAIN_DATA_PATH = os.getenv("TRAIN_DATA_PATH", "/path/to/train.txt")
...
SWANLAB_PROJECT = os.getenv("SWANLAB_PROJECT", "Video-Distillation-StructureConv")
SWANLAB_RUN_NAME = f"final_240W_conv_var4_resume"
...
CLIPS_PER_VIDEO = 2      
EFFECTIVE_BATCH_SIZE = BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS * CLIPS_PER_VIDEO  # 有效batch size = 16
EPOCHS = 30
NUM_FRAMES = 8           
...
LEARNING_RATE = 1.2e-3  # 保持或略微增大，见下方建议
NUM_WORKERS = 8
...
def main():
    ...
    if rank == 0:
        print(f"Start Training: {EPOCHS} epochs, now at epoch {start_epoch}.") # 第18个epoch
    ...
    if RESUME_PATH is None:
        ...
        global_step = len(train_dataloader) // GRADIENT_ACCUMULATION_STEPS * start_epoch
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if rank == 0:
            ...
            swanlab.log({"train/avg_train_loss": avg_train_loss}, step=epoch)
            ...
            swanlab.log(swanlab_log_dict, step=epoch)
